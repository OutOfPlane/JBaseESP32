...
jFramework::TCA9534::TCA9534(const char *name, i2cPort *port)
...
jFramework::TCA9534::~TCA9534()
{
}

j_err jFramework::TCA9534::setPinDirection(uint8_t pin, pinDirection dir)
{
...
bool jFramework::TCA9534::pinDirectionSupported(uint8_t pin, pinDirection dir)
{
    return ((pin < 8) && ((dir == PIN_INPUT) || (dir == PIN_OUTPUT)));
}

j_err jFramework::TCA9534::setPin(uint8_t pin, uint8_t val)
{
    j_err erg = J_ERR_TIMEOUT;
    if (_port->lock(*this, 10))
    {
        if (val)
        {
...
j_err jFramework::TCA9534::getPin(uint8_t pin, uint8_t &val)
{
    j_err erg = J_ERR_TIMEOUT;
    if (_port->lock(*this, 10))
    {
...
bool jFramework::TCA9534::canSetPin(uint8_t pin)
{
    return (pin < 8);
}

bool jFramework::TCA9534::canGetPin(uint8_t pin)
{
    return (pin < 8);
}

ioPin *jFramework::TCA9534::getIOPin(uint8_t pin)
{
    if (pin < 8)
    {
        return _pins[pin];
