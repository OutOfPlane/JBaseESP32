...
j_err esp32uartPort::w(uint8_t *data, uint16_t len)
{
    J_LOGI("writing: %.*s\r\n", len, (char *)data);
...
j_err esp32uartPort::r(uint8_t *data, uint16_t &len, uint32_t timeout_ms)
{
...
j_err esp32uartPort::present(bool &isPresent)
{
    isPresent = true;
    return J_OK;
}
