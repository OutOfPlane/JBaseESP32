...
j_err captivePortal::start()
{
    ...
    /** HTTP server */
    J_LOGI("Starting HTTP Server...");

    httpd_config_t config = HTTPD_DEFAULT_CONFIG();
    config.uri_match_fn = httpd_uri_match_wildcard;
    config.lru_purge_enable = true;

    erg = j_err_translate(httpd_start(&_server, &config));
    if (erg != J_OK)
        return erg;

    J_LOGI("Started HTTP Server.");

    J_LOGI("Registering HTTP server URI handlers...");

    /** use 404 Handler for redirects URI handler */
    httpd_register_err_handler(_server, HTTPD_404_NOT_FOUND, _commonHandler);

    J_LOGI("Registered HTTP server URI handlers.");

    return J_OK;
}

void jFramework::captivePortal::stop()
{
}

j_err jFramework::captivePortal::onGet(const char *path, esp_err_t (*handler)(httpd_req_t *r))
{
    /** URI handler */
    ...
    return j_err_translate(httpd_register_uri_handler(_server, &common_get_uri));
}

j_err jFramework::captivePortal::onPost(const char *path, esp_err_t (*handler)(httpd_req_t *r))
{
    /** URI handler */
    ...
    return j_err_translate(httpd_register_uri_handler(_server, &common_post_uri));
}

void jFramework::captivePortal::_dns_task(void *args)
{
    ...
    do
    {
        ...
        if (prtl->_sock_fd == -1)
        {
            J_LOGE("dns_task failed to create sock!, retrying...");
    ...
    do
    {
        ...
        if (ret != 0)
        {
            J_LOGE("dns_task failed to bind sock!");
            vTaskDelay(1000 / portTICK_RATE_MS);
        }
    } while (ret != 0);

    J_LOGI("DNS initialized.");
...
esp_err_t jFramework::captivePortal::_commonHandler(httpd_req_t *req, httpd_err_code_t error)
{
    ...
    if (res != ESP_OK)
    {
        J_LOGE("failed getting HOST header value: %d", res);

        switch (res)
        {
        case ESP_ERR_NOT_FOUND:
            J_LOGE("failed getting HOST header value: ESP_ERR_NOT_FOUND: Key not found: %d", res);
            break;

        case ESP_ERR_INVALID_ARG:
            J_LOGE("failed getting HOST header value: ESP_ERR_INVALID_ARG: Null arguments: %d", res);
            break;

        case ESP_ERR_HTTPD_INVALID_REQ:
            J_LOGE("failed getting HOST header value: ESP_ERR_HTTPD_INVALID_REQ: Invalid HTTP request pointer: %d", res);
            break;

        case ESP_ERR_HTTPD_RESULT_TRUNC:
            J_LOGE("failed getting HOST header value: ESP_ERR_HTTPD_RESULT_TRUNC: Value string truncated: %d", res);
            break;
    ...
    J_LOGI("Got HOST header value: %s", req_hdr_host_val);
    ...
    if (strncmp(req_hdr_host_val, correct_host, strlen(correct_host)) != 0)
    {
        ...
        J_LOGI("Detected redirect trigger HOST: %s", req_hdr_host_val);
    ...
    {
        J_LOGI("No redirect needed for HOST: %s", req_hdr_host_val);
...
j_err jFramework::captivePortal::_dns_recv(sockaddr_in *premote_addr, char *pusrdata, unsigned short length)
{
    ...
    if (length > WIFI_CAPTIVE_PORTAL_ESP_IDF_DNS_LEN)
        return J_ERR_INTERNAL; // Packet is longer than DNS implementation allows
    if (length < sizeof(DnsHeader))
        return J_ERR_INTERNAL; // Packet is too short
    if (hdr->ancount || hdr->nscount || hdr->arcount)
        return J_ERR_INTERNAL; // this is a reply, don't know what to do with it
    if (hdr->flags & WIFI_CAPTIVE_PORTAL_ESP_IDF_DNS_FLAG_TC)
        return J_ERR_INVALID_STATE; // truncated, can't use this
    ...
    for (i = 0; i < ntohs(hdr->qdcount); i++)
    {
        ...
        J_LOGI("DNS: WIFI_CAPTIVE_PORTAL_ESP_IDF_DNS_Q (type 0x%X cl 0x%X) for %s\n", ntohs(qf->type), ntohs(qf->cl), buff);
