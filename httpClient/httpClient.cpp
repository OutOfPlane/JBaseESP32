#include "httpClient.hpp"
#include "esp_netif.h"
#include "esp_crt_bundle.h"

const char *cert = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----)EOF";

using namespace jFramework;

jFramework::httpClient::httpClient(const char *name)
    : jObject(name)
{
    memset(&_config, 0, sizeof(_config));
    _config.url = "http://www.google.de";
    // _config.crt_bundle_attach = esp_crt_bundle_attach;
    _config.cert_pem = cert;
    _client = esp_http_client_init(&_config);
}

j_err jFramework::httpClient::get(char *responseBuffer, uint16_t responseBufferSize, const char *requestUrl)
{
    int content_length = 0;
    esp_http_client_set_url(_client, requestUrl);
    esp_http_client_set_method(_client, HTTP_METHOD_GET);

    esp_err_t err = esp_http_client_open(_client, 0);
    if (err != ESP_OK)
    {
        J_LOGE("Failed to open HTTP connection: %s", esp_err_to_name(err));
        return J_ERR_NETWORK;
    }
    else
    {
        content_length = esp_http_client_fetch_headers(_client);
        if (content_length < 0)
        {
            J_LOGE("HTTP client fetch headers failed");
            esp_http_client_close(_client);
            return J_ERR_INVALID_RESPONSE;
        }
        else
        {
            int data_read = esp_http_client_read_response(_client, responseBuffer, responseBufferSize);
            if (data_read >= 0)
            {
                responseBuffer[data_read] = 0;
                J_LOGI("HTTP GET Status = %d, content_length = %d",
                       esp_http_client_get_status_code(_client),
                       esp_http_client_get_content_length(_client));
            }
            else
            {
                responseBuffer[0] = 0;
                J_LOGE("Failed to read response");
                esp_http_client_close(_client);
                return J_ERR_NO_DATA;
            }
        }
    }
    esp_http_client_close(_client);

    return J_OK;
}

const char *hexChars = "0123456789ABCDEF";

j_err jFramework::httpClient::getPrintf(char *responseBuffer, uint16_t responseBufferSize, const char *requestUrl, ...)
{
    char tmp[256];
    va_list args;
    va_start(args, requestUrl);
    vsprintf(tmp, requestUrl, args);
    va_end(args);
    // urlencode the getPARAMs
    char tmpENC[256];
    char *tmpPtr = tmp;
    char *tmpENCPtr = tmpENC;
    uint8_t paramStart = 0;
    while (*tmpPtr)
    {
        if(*tmpPtr == 0x1B)
            break;
        *tmpENCPtr = *tmpPtr;
        if (paramStart)
        {
            if(*tmpPtr == '=')
                *tmpENCPtr = *tmpPtr;
            else if(*tmpPtr == '&')
                *tmpENCPtr = *tmpPtr;
            else if(isalnum(*tmpPtr) == 0)
            {
                *tmpENCPtr = '%';
                tmpENCPtr++;
                *tmpENCPtr = hexChars[((*tmpPtr) >> 4) & 0xF];
                tmpENCPtr++;
                *tmpENCPtr = hexChars[((*tmpPtr) >> 0) & 0xF];
            }
        }
        if (*tmpPtr == '?')
            paramStart = 1;
        tmpPtr++;
        tmpENCPtr++;
    }
    *tmpENCPtr = '\0';
    J_LOGI("Executing request: %s", tmpENC);
    return get(responseBuffer, responseBufferSize, tmpENC);
}
